...
WHERE  current_stage IN (SELECT id FROM tbl_recruitment_stage as rs WHERE archive = 0 AND stage_label_id = 5) AND `ra`.`archive` = 0 AND `ra`.`status` = 1 AND `ra`.`flagged_status` = 0 AND `ra`.`duplicated_status` = 0 
...
inner join tbl_recruitment_stage_label s_rsl on s_rsl.stage_number = s_rt.task_stage AND s_rsl.id = 5 where ra.id = s_rta.applicant_id AND s_rta.archive = 0 AND s_rta.status IN (1,0) having mark_as_no_show = 0 )
# AND ra.recruiter=10
GROUP BY `ra`.`id`;