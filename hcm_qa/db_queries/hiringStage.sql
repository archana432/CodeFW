...
FROM `tbl_recruitment_applicant` as `ra` INNER JOIN `tbl_recruitment_applicant_email` as `rae` ON `rae`.`applicant_id` = `ra`.`id` AND `primary_email` = 1 
INNER JOIN `tbl_recruitment_applicant_phone` as `rap` ON `rap`.`applicant_id` = `ra`.`id` AND `primary_phone` = 1 
WHERE current_stage IN (SELECT id FROM tbl_recruitment_stage as rs WHERE archive = 0 AND stage_label_id = 7) AND `ra`.`archive` = 0 AND `ra`.`status` = 1 AND `ra`.`flagged_status` = 0 AND `ra`.`duplicated_status` = 0 
...
inner join tbl_recruitment_stage_label s_rsl on s_rsl.stage_number = s_rt.task_stage AND s_rsl.id = 7 
